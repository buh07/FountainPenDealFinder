...
class RakumaAdapter:
    """Best-effort connector for Rakuten Rakuma search pages."""

    def __init__(self) -> None:
        settings = get_settings()
        self.base_url = settings.rakuma_base_url.rstrip("/")
        self.search_path = settings.rakuma_search_path
        self.default_keyword = settings.rakuma_keyword
        self.max_results = max(1, settings.rakuma_max_results)
        self.timeout = settings.rakuma_timeout_seconds
        self.verify_ssl = settings.rakuma_verify_ssl
        self.request_interval_seconds = max(0.0, settings.rakuma_request_interval_seconds)
    ...
    def _search_internal(self, keyword: str) -> list[RawListingPayload]:
        html = self._request(self.search_path, params={"query": keyword})
        soup = BeautifulSoup(html, "html.parser")

        rows = self._parse_ldjson_itemlist(soup)
        if not rows:
            rows = self._parse_anchor_items(soup)
        ...
        parsed = list(deduped.values())[: self.max_results]
        return [self._repair_price_with_detail(row) for row in parsed]
    ...
    def fetch_listing_detail(self, source_id: str) -> RawListingPayload | None:
        ...
        for image in soup.select("img[src]"):
            ...
            if src and ("rakuma" in src or "fril" in src):
